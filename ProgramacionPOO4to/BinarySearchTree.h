...
public:
	...
	bool Search(T value)
	{
		Node* nodoActual = root; //Comenzamos desde la raíz

		while (nodoActual != nullptr)
		{
			//SI ENCONTRAMOS EL VALOR
			//comparamos el valor buscado con el valor del nodo en el que nos encontramos
			if (value == nodoActual->value)
			{
				return true; //tenemos el valor y sabemos que esta en nuestro arbol
			}
			//EL VALOR BUSCADO ES MENOR
			//los valores que son menores siempre están a la izquierda de nuestro arbol
			else if (value < nodoActual->value)
			{
				nodoActual = nodoActual->leftChild; //nos movemos hacia el hijo izquierdo deñll arbol
			}
			//EL VALOR BUSCADO ES MAYOR
			//los valores que son mayores siempre están a la derecha de nuestro arbol
			else
			{
				nodoActual = nodoActual->rightChild; //nos movemos hacia el hijo derecho del arbol
			}
		}
		//si nuestro bucle while termina significa que el nodoActual se volvió un nullptr por lo tanto recorremos hasta encontrar el valor
		return false; 
	}
	//--------------------------------------------> FUNCION Search <--------------------------------------------
	...
	{
		DeletePostOrder(root); //llamamos a la función recursiva comenzando desde la raiz
		root = nullptr; 
	}
	//---------------------------------> EJERCICIO 3 <---------------------------------
	...
	void PostOrderIterativo()
	{
		...
		stack<Node*> pila; //creamos una pila para guardar el camino recorrido
		Node* actual = root; //creamos un puntero que sera el que se moverá
		Node* ultimo = nullptr; //esto nos ayudara para guardar cual fue el ultimo nodo por el que pasamos y asi no repetir camino

		while (!pila.empty() || actual != nullptr) //seguiremos mientras tengamos nodos en la pila o hasta que el explorador no sea nulo
		{
			if (actual != nullptr) //vamos a bajar por la izquierda todo lo que sea posible
			{
				pila.push(actual); //guardamos el nodo antes de bajar a lo ultimo
				actual = actual->leftChild; //nos vamos moviendo a la izquierda
			}
			else
			{
				Node* tope = pila.top(); //llegamos al tope actual es null y miramos al padre en la pila

				//validamos si el nodo tiene un hijo derecho y si a ese hijo derecho no es el que acabamos de visitar
				if (tope->rightChild != nullptr && ultimo != tope->rightChild)
				{
					//si entramos significa que hay camino el cual aun no se a visitado 
					actual = tope->rightChild; //y nos novemos hacia la derecha
				}
				else
				{
					//entraremos aqui si no hay hijo derecho o si ya visitamos al hijo derecho
					cout << tope->value << ", "; 
					ultimo = tope; //marcaremos el nodo como ya visitado 
					pila.pop(); //y lo sacaremos de la pila definitivamente 
				}
			}
		}
	}
...
private:
	...
	//--------------------------------------------> EJERCICIO 3 <--------------------------------------------
	void DeletePostOrder(Node* nodo)
	{
		if (nodo == nullptr) return; //si nuestro nodo es nulo no hay nada que borrar y regresamos

		DeletePostOrder(nodo->leftChild); //primero bajaremos hasta el fondo de la parte izquierda del arbol

		DeletePostOrder(nodo->rightChild); //luego bajaremos hasta el fondo pero ahora de la parte derecha del arbol

		//una vez que los hijos (izq y der) han sido eliminados estaremos listos para poder borrar el nodo actual
		nodo->leftChild = nullptr;
		nodo->rightChild = nullptr;
		nodo->parent = nullptr;
		delete nodo;
	}
	//--------------------------------------------> EJERCICIO 3 <--------------------------------------------
