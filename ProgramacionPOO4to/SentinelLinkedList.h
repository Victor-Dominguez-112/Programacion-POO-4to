...
public:
	...
	~SentinelLinkedList()
	{
		...
		delete NIL;
		cout << "Sentinel linked list destruida con exito" << endl;
	}
	...
	{
		Node* newNode = new Node(value); //El nuevo nodo apunta hacia el primer nodo actual que hay
		newNode->next = NIL->next; // El primer nodo actual apuntara hacia atrás al nuevo nodo
		NIL->next->prev = newNode; // El NIL ahora apunta hacia adelante al nuevo nodo
		NIL->next = newNode; // El nuevo nodo apuntara hacia atrás al NIL
		newNode->prev = NIL;
		count++;
	}
